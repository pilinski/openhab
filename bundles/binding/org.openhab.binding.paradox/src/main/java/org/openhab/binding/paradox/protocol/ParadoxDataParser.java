 ...
 * @since 1.5.0
 */
public class ParadoxDataParser {

	private HashMap<String, ParadoxDataParserRule> parsingRules = null;

	public ParadoxDataParser(
			HashMap<String, ParadoxDataParserRule> parsingRules) {

		this.parsingRules = parsingRules;
	}

	/**
	 * Method to parse Paradox device datagram to variables by defined parsing rules.
	 * 
	 * @param data datagram from Paradox device
	 * 
	 * @return Hash table which contains all parsed variables.
	 */
	public HashMap<String, Number> parseData( String data) {

		HashMap<String, Number> variables = new HashMap<String, Number>();

		byte[] address = data.getBytes();

		for (Entry<String, ParadoxDataParserRule> entry : parsingRules
				.entrySet()) {

			ParadoxDataParserRule rule = entry.getValue();

			if (rule.getZoneState() == 0) {
